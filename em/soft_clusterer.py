...
class Soft_Clustering:
    def __init__(self, k, data):
        ...
        self.means      = np.zeros(shape=(k,), dtype=np.float32)
        self.priors     = np.zeros(shape=(k,), dtype=np.float32)
        self.variances  = np.zeros(shape=(k,), dtype=np.float32)
    ...
    def maximize(self):
        ...
        # priors
        self.priors = effective_data_size / self.n
        
        # means
        self.means = (self.data[None, :] @ self.class_prob)[0] / np.sum(self.class_prob, axis=0)
        ...
        self.variances = (numerator / denominator).astype(np.float32)
